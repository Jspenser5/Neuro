...
from src.pycochleagram.pycochleagram import utils
...
def make_lin_cos_filters(signal_length, sr, n, low_lim, hi_lim, full_filter=False, strict=False):
  """Fairly literal port of Josh McDermott's MATLAB make_lin_cos_filters. Useful
  for debugging, but isn't very generalizable. Use make_erb_cos_filters_1x or
  make_erb_cos_filters_nx with sample_factor=1 instead.
  ...
  # cutoffs are evenly spaced on an linear scale
  ...
  for k in range(n):
    ...
    cos_filts[l_ind:h_ind+1,k] = np.cos((freqs[l_ind:h_ind+1] - avg)/rnge * np.pi)  # h_ind+1 to include endpoint
